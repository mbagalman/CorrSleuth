...
| `OSCILLATION_BIN_LOF_FLOOR` | 0.3 | `bin_lof_r2_gain` floor that must hold **jointly** with `OSCILLATION_MIN_REVERSALS` before the reversal count is trusted | Deliberately far above `BIN_LOF_R2_GAIN_THRESHOLD` (0.05): the reversal count is only meaningful given substantial bin structure. This floor is what excludes pure noise, which produces *more* raw reversals than a real sinusoid (measured 13–16 vs. 3–4 on blind test data) but a gain ~15× smaller (0.057 vs. 0.826). Locked via a 2,080-run sweep (13 shapes × 4 sample sizes × 4 noise levels × 10 seeds, plus a 6-cycle-count sinusoid grid): zero false positives; detection 10/10 everywhere except 3–5 cycles at n=100 under heavy noise, where ~10 bins genuinely cannot resolve the cycles. Real sinusoids measured gains 0.3–0.97. |
