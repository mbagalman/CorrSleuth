...
| `OSCILLATION_MIN_REVERSALS` | 2 | minimum `bin_reversal_count` (bin-mean direction reversals, counted with hysteresis — see `metrics/shape.py`) for the oscillation route into `nonmonotonic_dependence` and the `dependence_type = oscillating` axis value | A single bend (U-shape) measures exactly 1 reversal, so 2 is the smallest count that separates oscillation from it; a 1.5-cycle sinusoid measured ≥ 2 (usually 3) across the validation sweep. Never applied alone — always jointly with the gain floor below. |
